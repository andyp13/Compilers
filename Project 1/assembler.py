...
class hackAssembler:
        ...
        for index, currentLine in enumerate(currentFile):
            ...
            try: 
                if(len(currentLine)):
                    ...
                    elif(currentLine.lower().startswith('x')):
                        # Looking at an X Type
                        newLine = self.xType(index, currentLine)
                    else:
    ...
    def xType(self, lineNumber, line:str):
        ...
        if(representsHex('0'+ line)): # Make sure it is hex and convert it to an int
            ...
            output = cDefinition | command
            return bin(output)[2:].zfill(16)  # Convert to binary string
        else:
    ...
    def cType(self, lineNumber, line : str):
        ...
        destText = line.split('=')[0] if '=' in line else None
        jumpText = line.split(';')[1] if ';' in line else None
        destIndex = line.find('=')
        jumpIndex = line.find(';')
        compText = line[destIndex + 1 if destIndex != -1 else 0 : jumpIndex if jumpIndex != -1 else len(line)]
...
for filePath in sys.argv[1:]:
    ...
    newFile.close()
    print(f'Outputed to -> {newFileName}')
    ...
    newTableFile.close()
    print(tableFile)
    print(f'Also Written To -> {tableFileName}')
